...
该模块是 Fustor 平台的自动化压力测试和性能量化工具。它通过模拟大规模文件系统元数据，量化 fustord API 相比于操作系统原生文件系统调用的性能优势。

## 核心设计目标

1.  **量化优势**: 对比 fustord 内存索引与 Linux 原生 `find` 命令在递归元数据检索下的延迟与吞吐量。
2.  **百万级规模**: 支持生成并同步超过 1,000,000 个文件的元数据。
3.  **全自动流程**: 自动编排 fustord 和 sensord，实现一键式从环境部署到报告生成。
...
### 2. 执行压测 (全自动模式)
此模式会自动在本地启动 fustord 和 sensord，并在压测结束后自动停止：
```bash
...
uv run fustor-benchmark run /mnt/nfs_data \
    --fustord-api http://10.0.0.1:18102 \
...
| `--fustord-api` | - | - | 外部 fustord API 地址（跳过本地服务启动） |
| `--api-key` | - | - | 外部 fustord API 的身份验证 Key |
...
1.  **`stress-find.html`**: 交互式可视化报表。
    ...
    *   **Speedup Factor**: 自动计算 fustord 相比 OS 的加速倍数。
