...
3.  **Fusion API (Perception Mode)**: Fusion 核心检索。由于 sensord 位于写入端侧，通过实时捕获文件系统关闭事件（Close-on-Write）来实现状态感知，Fusion 无需任何静默等待即可返回 100% 完整且可靠的数据。无论文件是否正在写入，其延迟和吞吐量始终保持稳定。
