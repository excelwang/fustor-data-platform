...
class fustordClient:
    def __init__(
        ...
        """
        Initialize fustordClient.
        
        Args:
            base_url: The base URL of the fustord service
    ...
    async def get_fustord_config(self, filename: str = "default.yaml") -> Dict[str, Any]:
        """Return the current fustord configuration."""
        response = await self.client.get(f"{self._management_path}/config", params={"filename": filename})
        response.raise_for_status()
        return response.json()

    async def update_fustord_config_structured(self, config: Dict[str, Any], filename: str = "default.yaml") -> Dict[str, Any]:
        """Update fustord config via structured JSON."""
        payload = {**config, "filename": filename}
        response = await self.client.post(f"{self._management_path}/config/structured", json=payload)
        response.raise_for_status()
        return response.json()

    async def get_DatacastConfig(self, datacast_id: str, trigger: bool = False, filename: str = "default.yaml") -> Dict[str, Any]:
    ...
    async def update_DatacastConfig_structured(self, datacast_id: str, config: Dict[str, Any], filename: str = "default.yaml") -> Dict[str, Any]:
    ...
    async def reload_fustord(self) -> Dict[str, Any]:
        """Trigger a hot-reload of fustord configuration."""
    ...
    async def push_events(self, session_id: str, events: List[Dict[str, Any]], source_type: str, is_snapshot_end: bool = False, metadata: Optional[Dict[str, Any]] = None) -> bool:
        """
        Pushes a batch of events to the fustord service.
        """
        try:
    ...
    async def terminate_session(self, session_id: str) -> bool:
        """
        Terminates the given session on the fustord service.
        """
        try:
