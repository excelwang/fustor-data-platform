...
class TestOnCommandFallback:
    ...
    def test_fallback_mechanism_works(
        ...
        assert fustord_client.wait_for_datacast_ready("client-a", timeout=SHORT_TIMEOUT),datacastcast A not ready"
        ...
        datacast_log = docker_manager.exec_in_container(CONTAINER_CLIENT_A, ["cat", "/root/.fustor/logdatacastcast.log"]).stdout
        # PipeCommandMixin logs: "Received command 'scan'"
        assert "Received command 'scan'" in datacast_log,datacastcast should have received 'scan' command (Verify P0-3 fix)"
