...
class TestHeartbeatTimeout:
    ...
    def test_datacast_recovers_after_timeout(
        ...
        datacast_a = next((s for s in sessions if "client-a" in s.get("datacast_id", "")), None)
        assert datacast_a is not None, "datacast A must have a session initially"
        ...
        while time.time() - start_wait < MEDIUM_TIMEOUT:
            ...
            for pattern in fatal_patterns:
                if pattern in logs:
                    logger.error(f"datacast A CRITICAL ERROR detected in datacast.log:\n{logs}")
