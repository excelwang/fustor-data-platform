...
验证无 datacast 客户端创建的文件通过 Audit 被发现，并标记为 Datacast_missing。
...
class TestBlindSpotFileCreation:
    """Test detection of files created by client without datacast."""
    ...
    def test_blind_spot_file_discovered_by_audit(
        ...
        # Step 1: Create file on client without datacast
        ...
        # Verify file exists on datacast A's mount (check NFS propagation)
        print(f"[DEBUG] Waiting for file to be visible on datacast A (NFS propagation)...")
        visible_on_a = False
        for _ in range(20): # ~10s wait
            ...
            if check_a.returncode == 0:
                visible_on_a = True
                print(f"[DEBUG] File visible on datacast A: {check_a.stdout.strip()}")
                break
            time.sleep(0.5)
        
        assert visible_on_a, f"File {test_file} did not become visible on datacast A within timeout"
        ...
        # IMPORTANT: Wait for datacast A to see the changes via NFS actimeo
        print(f"[DEBUG] Waiting {INGESTION_DELAY}s for NFS propagation to datacast A...")
        time.sleep(INGESTION_DELAY)

        # Check dir mtime from datacast A's perspective
        try:
            dir_stat = docker_manager.exec_in_container(CONTAINER_CLIENT_A, ["stat", MOUNT_POINT])
            print(f"[DEBUG] datacast A dir stat: {dir_stat.stdout}")
            file_stat = docker_manager.exec_in_container(CONTAINER_CLIENT_A, ["ls", "-la", test_file])
            print(f"[DEBUG] datacast A file check: {file_stat.stdout}")
        except Exception as e:
            print(f"[DEBUG] datacast A check failed: {e}")
        ...
        # full audit cycle started AFTER the file was visible on datacast A.
