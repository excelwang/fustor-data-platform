"""
Test A1: First datacast becomes Leader.

验证第一个连接到 fustord 的 datacast 被选举为 Leader。
...
class TestLeaderElectionFirst:
    """Test that the first datacast to connect becomes the leader."""

    def test_first_datacast_becomes_leader(
        self,
        docker_env,
        fustord_client,
        setup_datacasts,
        ...
        """
        场景: 第一个 datacast A 连接到 fustord
        预期: datacast A 被选举为 Leader
        验证方法: 查询 fustord Sessions API，确认 datacast A 的 role 为 "leader"
        """
        ...
        test_logger.info(f"Elected leader for view: {leader_session.get('datacast_id')}")
