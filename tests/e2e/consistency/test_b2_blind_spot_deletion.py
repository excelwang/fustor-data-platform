...
验证无 datacastst 客户端删除的文件通过 Audit 被发现，并从内存树移除。
...
class TestBlindSpotFileDeletion:
    """Test detection of files deleted by client without datacastst."""
    ...
    def test_blind_spot_deletion_detected_by_audit(
        ...
        """
        场景:
          1. datacastst A 创建文件（正常路径，实时同步到 fustord）
          2. 无 datacastst 的客户端 C 删除该文件
          3. 因为 C 没有 datacastst，删除事件不会实时同步
        ...
        # Step 1: Create file from datacastst client (realtime sync)
    ...
    def test_blind_spot_deletion_added_to_blind_spot_list(
        ...
        # Create file from datacastst, then delete from blind-spot
        docker_manager.create_file_in_container(
            CONTAINER_CLIENT_A,
            test_file,
