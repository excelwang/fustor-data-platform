...
│  │  │  (datacast A)   │  │ datacastcast B)   │  │  datacasttacast)  │            │ │
...
### 2. 代码生效机制
- **热生效**: 核心代码（`datacast_core`,datacastcast`, `fustord` 等）已通过 **Volume 挂载**。修改 `src` 目录代码后，测试固件会自动重启进程使代码生效，**无需重启容器**。
- **三方库变更**: 修改了 `pyproject.toml` 中的 `dependencies` 后，系统会自动触发镜像重建。

### 3. 排障与日志
测试失败时，可以通过以下命令查看实时日志：
- **fustord 日志**: `docker logs -f fustord`
- **datacast 日志**: `docker exec client-a cat /tmdatacastcast.log`
