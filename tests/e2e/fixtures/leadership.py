...
def leader_follower_datacasts(setudatacastcasts, fustord_client):
    """
    ...
    if leader and "client-a" in leader.get("datacast_id", ""):
        # datacast A is leader. Checdatacastcast B presence.
        datacast_b = next((s for s in sessions if "client-b" in s.getdatacastcast_id", "")), None)
        if datacast_b andatacastcast_b.get("role") == "follower":
            is_clean = True
