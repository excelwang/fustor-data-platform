# L3: [decision] [sensord] Configuration Pattern
...
### 1.1 The `sensord_config` Loader
- **Location**: `sensord.config.unified.sensord_config`
...
由于使用了全局单例，测试必须显式 mock `sensord_config`：

```python
with patch("sensord.config.unified.sensord_config") as mock_conf:
