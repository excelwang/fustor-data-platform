...
## 3. 消费者端一致性机制 (Consumer Logic)

**Sensord Data Protocol (SDP)** 允许消费者实现以下高阶一致性逻辑：

### 3.1 墓碑机制 (Tombstones)
- **场景**: 处理文件删除。
- **逻辑**: 消费者在收到删除事件时并不物理删除记录，而是标记为 Tombstone。
- **转世**: 若收到相同路径的新事件且 `mtime` 大于 Tombstone 时间戳，文件“转世”为新记录。

### 3.2 疑似状态 (Suspect Mode)
- **场景**: 处理非原子写入（如正在拷贝中的大文件）。
- **逻辑**: 若文件的 `mtime` 极度接近当前逻辑水位线，将其标记为 `Suspect`。
- **清理**: 只有当水位线推进且后续审计通过后，Suspect 状态才会被解除。

### 3.3 盲区检测 (Blind-spot Detection)
- **场景**: 补全因 `inotify` 队列溢出丢失的事件。
- **逻辑**: 通过 Audit 扫描结果与本地视图进行 `diff`，自动发现并同步从未见过的文件。
...
1.  **实时优先**: 实时产生的 `realtime` 事件具有最高权威。
2.  **强制补偿**: 通过定期 `audit` 扫描发现由于监控盲区或重启导致的遗漏。
---
...
### 1.1 目标场景 (Target Scenario)

多台计算服务器通过共享存储（如 NFS/OSS）挂载同一目录。其中若干节点部署了 **sensord**，负责感知并同步数据变更至外部 **Consumer**（集线器、聚合器或管理平台）。

```mermaid
graph TD
    Consumer["Consumer / Aggregator (Consistency Arbitration)"]
    
    subgraph "Compute Fleet"
        SensordA["sensord Node A (Active ✅)"]
        SensordB["sensord Node B (Active ✅)"]
        SensordC["Silent Node C (Blind Spot ❌)"]
    end

    SharedStorage["Shared Storage (NFS / OSS)"]

    %% Connections
    SensordA <==>|Events/Heartbeat| Consumer
    SensordB <==>|Events/Heartbeat| Consumer
    SensordA --- SharedStorage
    SensordB --- SharedStorage
    SensordC --- SharedStorage
```
...
| **持久层滞后** | 不同客户端看到的共享存储状态可能有秒级甚至分钟级的缓存延迟 |
| **感知盲区** | 未部署 sensord 的节点产生的文件变更无法实时捕捉 |
| **时序冲突** | 全量扫描期间发生的并发实时变更可能导致数据矛盾 |

### 1.3 设计原则 (Design Principles)

1.  **实时优先**: 实时产生的 `realtime` 事件具有最高权威。
2.  **强制补偿**: 通过定期 `audit` 扫描发现由于监控盲区或重启导致的遗漏。
3.  **生存锁定**: 只有被分配为 **Leader** 的 sensord 实例执行全局性的全量/审计扫描。
4.  **影子参考系**: 实现漂移补偿计算，使本地物理时间戳对齐存储服务逻辑时间。

---

## 2. 角色分工 (Leader/Follower)

### 2.1 职责矩阵

| 行为 (Activity) | Leader sensord | Follower sensord |
|----------------|:--------------:|:----------------:|
| 实时同步 (Realtime Sync) | ✅ | ✅ |
| 全量快照 (Snapshot Sync) | ✅ | ❌ |
| 定时审计 (Audit Sync) | ✅ | ❌ |
| 稳定性轮询 (Sentinel Sweep) | ✅ | ❌ |

### 2.2 租约与选举 (Lease & Election)

sensord 不参与复杂的分布式共识协商（如 Raft），其角色由外部 **Consumer** 通过 **Heartbeat Response** 指派：
- 第一个接入并声明对该资源路径负责的 sensord 被指派为 Leader。
- sensord 必须服从 Consumer 的角色切换指令，并保持行为幂等。

---

## 3. 数据采集算法 (Data Collection)

### 3.1 Audit 快速扫描算法

利用 POSIX 语义：创建/删除文件通常会更新直接父目录的 `mtime`。

**True Silence (静默跳过)** 机制（由 `source-fs` 实现）：
...
def scan_dir(path):
    st = os.stat(path)
    if st.st_mtime == self.cache.get(path):
        # 目录静默：发送跳过标记，保护子项
        yield Event(path, audit_skipped=True, m_source="audit")
    else:
