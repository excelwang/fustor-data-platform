...
# L3: [runtime] [Sensord] Process Lifecycle
...
## 2. 脐带原则 (Heartbeat Principles)

**Sensord** 与消费者之间的 SCP 心跳不仅是为了探活，更是为了生存决策：

1.  **生存权置换**: 消费者在心跳响应中可以携带指令。如果心跳中断超过 `timeout_seconds`，Sensord **必须** 停止该 Session 下的数据推送并进入 `RECONNECTING` 状态。
...
- **晋升 (Promotion)**: 当感应到角色由 Follower 变为 Leader 时，sensord 必须：
    1. **清空 Audit 缓存**: 确保当选后的首轮审计是物理上的全量对账。
    2. **触发 Snapshot**: 补齐作为 Follower 期间可能缺失的资产全貌。
- **降级 (Demotion)**: 立即停止正在进行的物理扫描任务（Snapshot/Audit），仅维持 inotify 实时监听。

---

## 2. 心跳永续原则 (Heartbeat Never Dies)

**核心不变量**: 只要进程存活且任务启用，心跳循环 (Heartbeat Tunnel) 永远不应因业务 Task 的崩溃而终止。

1. **状态隐式上报**: 心跳包通过 `can_realtime` 等健康位隐式告知 Consumer 当前数据面的就绪状态。
2. **命令隧道**: 心跳响应是 Consumer 下发远程指令（如 `upgrade`, `scan`）的唯一实时通道。
3. **退避策略 (Backoff)**: 当发生连接错误时，心跳必须执行指数级退避（Exponential Backoff），严禁因重连失败而彻底退出。

---

## 3. 审计上下文生命周期 (Audit Context)
