...
### 2.2 漂移补偿 (Drift Compensation)
- **Watermark 推进**: 每一次 SCP 心跳会携带消费者的当前全局水位值。
- **修正公式**: `Local_Watermark = Max(Internal_Clock, Consumer_Watermark - Drift_Threshold)`。

---

## 3. 约束 (Constraints)

1.  **非绝对时间性**: 消费者应当将事件中的 `logical_timestamp` 视为排序键，而非真实物理时间。
2.  **容忍回退**: 只有在 Audit 周期且 `force_sync=True` 时，才允许重新校准参考系原点。

### 2.1 漂移采样与补偿 (Sampling & Compensation)

Sensord 不盲目信任本机 `time.time()`，而是在启动或执行全量扫描时通过采样建立物理参考对齐。

**采样逻辑 (P99 Stability)**：
1. 收集目标存储路径中所有目录的 `mtime`。
2. 排序并选取 P99 分位点作为 `latest_mtime_stable`（有效排除未来的极端跳变干扰）。
3. 计算物理漂移值：`drift = latest_mtime_stable - time.time()`。
4. **影子映射**: 此后所有外发事件的逻辑时间轴均基于 `Local_Time + drift`。

### 2.2 事件索引生成契约 (Index Generation Contract)
