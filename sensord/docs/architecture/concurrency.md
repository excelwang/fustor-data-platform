# Fustor sensord 异步与并发设计

## 1\. 核心挑战

Fustor sensord 的核心是基于 `asyncio` 构建的异步应用，它必须能同时高效地处理 Web API 请求、管理多个Pipe 任务的生命周期以及执行内部逻辑。`asyncio` 的高效运行依赖于一个核心原则：**任何任务都不能长时间阻塞事件循环**。
...
本文档详细阐述了 Fustor sensord 如何通过分层的并发模型来优雅地解决这一系列问题。
...
为优化资源利用，Fustor sensord 引入了基于“源签名”的事件总线共享机制。如果多个Pipe 任务实际指向同一个物理数据源，它们将共享同一个 `EventBusInstanceRuntime`，从而避免重复建立连接和拉取数据。
