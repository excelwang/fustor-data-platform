# Fustor sensord 数据流设计

## 1. 引言

本文档详细描述了 Fustor sensord 内部数据流的设计，旨在解决两大核心挑战：
1.  **低延迟实时同步**: 如何确保新产生的数据能以最低延迟被同步？
2.  **数据完整性与恢复**: 如何在各种异常情况下（如初次同步、服务长期离线、源端数据丢失）确保数据最终一致性？

为解决这些问题，Fustor sensord 采用了一种先进的**“消息优先，自主并发快照”**架构。

## 2. 核心设计原则

1.  **消息优先 (Message-First)**: 系统的默认运行模式是**消息阶段 (Message Sync Phase)**。Pipe 任务一旦启动，会立即开始监听和推送实时数据流，确保新数据的同步延迟达到最小化。

2.  **会话状态管理 (Session-Based State)**: Fustor sensord 与消费端之间的所有交互都基于一个明确的**会话 (Session)**。任务启动时创建会话，并通过心跳维持。消费端负责持久化与该会话相关的同步点位，Fustor sensord 在启动时查询此点位以决定同步的起点。

3.  **自主并发回填 (Autonomous Concurrent Backfill)**: 当 Fustor sensord 发现源端无法满足消费端提供的起始点位时（即历史数据丢失），它会**自主地**、**并发地**启动一个后台快照任务来回填历史数据，**此过程不阻塞**主链路的实时消息阶段。

4.  **尽力而为 (Best-Effort Recovery)**: 当源驱动无法从请求的点位恢复时，它不会失败，而是会“尽力而为”地从最新的可用位置开始同步。这个“点位丢失”的信号会在 Fustor sensord 内部触发自主回填机制。
...
**解决方案**: **此问题的解决逻辑由一个“会话优先”的启动策略，和一个由sensord自主决策的、并发执行的“补充快照”机制共同完成。**
...
通过这种方式，系统保证了实时数据流的绝对优先和低延迟，同时提供了一个由 sensord 自身闭环控制的、对主流程无阻塞的、健壮的历史数据回填机制。
