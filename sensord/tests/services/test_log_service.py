...
def sample_log_content():
    return [
        "2025-07-08 10:00:00,123 - sensord.app - INFO - Application started.",
        "2025-07-08 10:00:01,456 - sensord.api - DEBUG - API call received.",
        "2025-07-08 10:00:02,789 - sensord.service - WARNING - Something unusual happened.",
        "2025-07-08 10:00:03,000 - sensord.driver - ERROR - Driver failed to connect.",
        "INVALID LOG LINE",
        "2025-07-08 10:00:04,111 - sensord.app - INFO - Application shutting down."
    ]

class TestLogService:
    def test_parse_valid_line(self, log_service):
        line = "2025-07-08 10:00:00,123 - sensord.app - INFO - Application started."
        entry = log_service._parse_line(line, 1)
        assert entry is not None
        assert entry.timestamp == datetime(2025, 7, 8, 10, 0, 0, 123000)
        assert entry.level == "INFO"
        assert entry.component == "sensord.app"
    ...
    def test_get_logs_with_component_filter(self, mock_file_open, mock_exists, log_service, sample_log_content):
        ...
        logs = log_service.get_logs(component="sensord.app")
        assert len(logs) == 2
        assert logs[0].component == "sensord.app"
        assert logs[1].component == "sensord.app"
        assert logs[0].message == "Application shutting down."
    ...
    def test_get_logs_with_all_filters(self, mock_file_open, mock_exists, log_service, sample_log_content):
        ...
        logs = log_service.get_logs(limit=1, level="INFO", component="sensord.app", before_line=6)
        assert len(logs) == 1
    ...
    def test_get_logs_io_error(self, mock_file_open, mock_exists, log_service):
        ...
        # with self.assertLogs('sensord', level='ERROR') as cm:
