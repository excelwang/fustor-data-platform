...
from sensord_sdk.interfaces import SourceDriverServiceInterface # Import the interface
...
class SourceDriverService(SourceDriverServiceInterface): # Inherit from the interface
    ...
    async def create_sensord_user(self, driver_type: str, **kwargs) -> Tuple[bool, str]:
        """Creates an sensord user for a given driver type by calling the class method."""
        try:
            driver_class = self._get_driver_by_type(driver_type)
            return await driver_class.create_sensord_user(**kwargs)
        except Exception as e:
            logger.error(f"Error during create_sensord_user for driver '{driver_type}': {e}", exc_info=True)
