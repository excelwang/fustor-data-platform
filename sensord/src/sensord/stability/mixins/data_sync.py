# sensord/src/sensord/runtime/pipe/data_sync.py
import asyncio
import logging
...
if TYPE_CHECKING:
    from ..pipe import SensordPipe

logger = logging.getLogger("sensord")

class PipeDataSyncMixin:
    """
    Mixin for SensordPipe data plane synchronization and supervision.
    """
    
    async def _run_data_supervisor_loop(self: "SensordPipe") -> None:
    ...
    async def _check_task_liveness(self: "SensordPipe", task: asyncio.Task, task_name: str) -> bool:
    ...
    async def _supervise_data_tasks(self: "SensordPipe") -> None:
    ...
    async def _cancel_all_tasks(self: "SensordPipe") -> None:
    ...
    async def _handle_fatal_error(self: "SensordPipe", error: Exception) -> None:
    ...
    async def _aiter_sync_phase(self: "SensordPipe", phase_iter: Iterator[Any], queue_size: Optional[int] = None, yield_timeout: Optional[float] = None):
    ...
    async def _run_message_sync(self: "SensordPipe") -> None:
    ...
    async def _run_bus_message_sync(self: "SensordPipe") -> None:
