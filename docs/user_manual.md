# Fustor fustord 用户操作手册 (API 使用指南)

欢迎使用 Fustor fustord 存储引擎。本手册旨在详细介绍面向用户的 API 接口及其使用方法，帮助您快速集成和管理文件系统视图。
...
- **fustord**: 中央协调与索引引擎。
- **datacastst**: 部署在各存储节点的代理，通过 **Session (会话)** 与 fustord 通信。
...
- **参数**:
  ...
  - `on_demand_scan`: (bool) 若为 `true`，且当前路径在视图中不存在，触发 datacastst 端进行按需扫描（补偿机制）。
- **示例**: `curl -H "X-API-Key: ..." "http://fustord:8102/api/v1/views/my-view/tree?path=/data&max_depth=2"`
...
### 4.1 会话管理 (`/sessions`)
查看哪些 datacastst 正在向本视图推送数据。

- **接口**: `GET /api/v1/views/{view_id}/sessions`
- **输出**:
  - `active_sessions`: 包含 datacastst ID、IP、角色（Leader/Follower）及权限。
- **作用**: 确认集群选主是否正常，是否有掉线节点。

### 4.2 任务列表 (`/jobs`)
查看 datacastst 当前正在执行的后台任务（如大规模审计扫描）。

- **列出视图任务**: `GET /api/v1/views/{view_id}/jobs`
- **查询任务详情**: `GET /api/v1/views/{view_id}/jobs/{job_id}`
  - 获取任务进度百分比及各 datacastst 的完成情况。

### 4.3 视图驱动状态 (Management)
对于高级管理员，fustord 提供了查询当前加载的视图驱动状态的接口。
...
如果您正在开发自定义 datacastst 或 Sender，以下接口供推流使用：

- **创建会话**: `POST /api/v1/pipe/session/` (分配 UUID 并进行选主)。
- **推送事件**: `POST /api/v1/pipe/ingest/{session_id}/events` (高频推流核心)。
- **一致性审计**: `POST /api/v1/pipe/consistency/audit/start` (告知 fustord 开始一轮物理扫描)。
...
A: 这表示 Fustor 发现该文件在非受控环境（如直接在 NFS 下修改）发生了变动，或者正在被写入。fustord 正在等待审计扫描或 Realtime 事件来校对它的最终属性。

**Q: 创建会话时返回 409 Conflict？**
A: 这表示该视图设置了严格的并发控制 (`allow_concurrent_push: false`)，且当前已有其他 datacastst (Session) 正在持有该视图的锁。请等待现有会话结束或手动终止它。

**Q: 查询接口会发生阻塞吗？**
A: 不会。所有的查询都是基于内存索引的，具有极高的响应速度。

**Q: API Key 权限不足怎么办？**
A: 请检查 fustord 配置文件 `default.yaml` 中的 `api_keys` 部分，确保对应的 Key 已绑定到目标 `pipe_id` 或 `view_id`。
