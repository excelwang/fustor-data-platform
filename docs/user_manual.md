...
- **fustord**: 中央协调与索引引擎。
- **datacast**: 部署在各存储节点的代理，通过 **Session (会话)** 与 fustord 通信。
...
- **参数**:
  ...
  - `on_demand_scan`: (bool) 若为 `true`，且当前路径在视图中不存在，触发 datacast 端进行按需扫描（补偿机制）。
...
### 4.1 会话管理 (`/sessions`)
查看哪些 datacast 正在向本视图推送数据。

- **接口**: `GET /api/v1/views/{view_id}/sessions`
- **输出**:
  - `active_sessions`: 包含 datacast ID、IP、角色（Leader/Follower）及权限。
- **作用**: 确认集群选主是否正常，是否有掉线节点。

### 4.2 任务列表 (`/jobs`)
查看 datacast 当前正在执行的后台任务（如大规模审计扫描）。

- **列出视图任务**: `GET /api/v1/views/{view_id}/jobs`
- **查询任务详情**: `GET /api/v1/views/{view_id}/jobs/{job_id}`
  - 获取任务进度百分比及各 datacast 的完成情况。
...
如果您正在开发自定义 datacast 或 Sender，以下接口供推流使用：
...
A: 这表示该视图设置了严格的并发控制 (`allow_concurrent_push: false`)，且当前已有其他 datacast (Session) 正在持有该视图的锁。请等待现有会话结束或手动终止它。
