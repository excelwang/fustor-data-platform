...
*   **架构**：1 个 datacast -> 1 个 fustord
*   **配置要点**：
    *   datacast 与 fustord 可部署在同一台机器或不同机器。
    *   无需配置复杂的选主策略。

### 1.2 高可用集群模式 (High Availability / HA Mode)
**适用场景**：生产环境的关键业务，要求服务不中断 (Zero Downtime)。

*   **架构**：N 个 datacast (监控同一存储) -> 1 个 fustord (集群)
*   **工作原理**：
    *   多个 datacast 均挂载相同的后端存储 (如 NFS/Ceph/NAS)。
    *   datacast 之间自动通过 fustord 进行协调，选举出一个 **Leader** (负责全量扫描与审计) 和多个 **Follower** (负责实时监听与冗余备份)。
    *   当 Leader 宕机时，Follower 会在秒级内自动接管，成为新的 Leader。
*   **配置要点**：
    *   所有 datacast 的 `source.uri` 必须指向逻辑上相同的文件系统内容 (即使挂载路径不同，Fustor 也能通过相对路径归一化处理)。
    *   建议配置 3 个以上的 datacast 节点以防止脑裂 (Split-Brain) 并提供更高的容错性。

### 1.3 聚合模式 (Aggregation Mode)
**适用场景**：统一数据湖视图、多部门数据汇总。

*   **架构**：多个 datacast (监控不同存储) -> 1 个 fustord (统一视图)
*   **工作原理**：
    *   datacast A 监控 `/data/department_a`
    *   datacast B 监控 `/data/department_b`
...
*   **架构**：1 个 datacast -> 1 个 fustord (Pipe) -> N 个 Views
*   **工作原理**：
    *   datacast 采集数据并通过 Pipe 发送到 fustord。
...
*   **allow_concurrent_push**:
    ...
    *   允许 datacast 并发推送事件，极大提高大量小文件变更时的同步吞吐量。
*   **session_timeout_seconds**:
    *   建议设置为 `3600` (1小时) 或更长。
    *   防止网络短暂波动导致 datacast 被误判下线，从而触发不必要的重新全量扫描。
