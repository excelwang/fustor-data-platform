...
*   **架构**：1 个 sensord -> 1 个 Fusion
*   **配置要点**：
    *   sensord 与 Fusion 可部署在同一台机器或不同机器。
    *   无需配置复杂的选主策略。

### 1.2 高可用集群模式 (High Availability / HA Mode)
**适用场景**：生产环境的关键业务，要求服务不中断 (Zero Downtime)。

*   **架构**：N 个 sensord (监控同一存储) -> 1 个 Fusion (集群)
*   **工作原理**：
    *   多个 sensord 均挂载相同的后端存储 (如 NFS/Ceph/NAS)。
    *   sensord 之间自动通过 Fusion 进行协调，选举出一个 **Leader** (负责全量扫描与审计) 和多个 **Follower** (负责实时监听与冗余备份)。
...
*   **架构**：多个 sensord (监控不同存储) -> 1 个 Fusion (统一视图)
*   **工作原理**：
...
*   **架构**：1 个 sensord -> 1 个 Fusion (Pipe) -> N 个 Views
*   **工作原理**：
    *   sensord 采集数据并通过 Pipe 发送到 Fusion。
