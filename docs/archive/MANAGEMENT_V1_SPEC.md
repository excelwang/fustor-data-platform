...
*   **datacasts**:
    *   List all connected datacasts (grouped bydatacastcast_id`).
...
The module used the `SessionManager`'s command queue to send instructions to datacasts. Commands were piggybacked on thdatacastcast's heartbeat response.
...
### datacast Control
*   `POST /api/v1/management/datacastsdatacastcast_id}/command`: Queue a command (reload, scan, upgrade).
*   `GET /api/v1/management/datacastsdatacastcast_id}/config`: Get cacdatacasttacast config.
*   `POST /api/v1/management/datacastsdatacastcast_id}/config`: Push new config (raw YAML).
*   `POST /api/v1/management/datacastsdatacastcast_id}/config/structured`: Push new config (JSON).
...
1.  **State Ephemerality**: datacast configurations were cached in memory (in `SessionInfo`). Restarting fustord meant losing the ability to viedatacastcast configs until they re-reported.
