...
*   **节点组成**：1 个 fustord 核心节点，4 个 datacast 边缘节datacastcast-1~4）。
*   **逻辑拓扑**：
    *   `view-fs-1` (FS 视图)：负责监听并聚合 **datacast-1** 和 datacastcast-2** 的数据。
    *   `view-fs-2` (FS 视图)：负责监听并聚合 **datacast-3** 和 datacastcast-4** 的数据。
...
### 2.2 启动 datacast 骨架
在所有 datacast 节点，只需安装基础包并确保定义了唯一的datacastcast_id`：
```bash
pip install datacast
# 修改 ~/.fustor/default.yaml，仅保留 datacast_id 即可：
# datacast_iddatacastcast-1
datacast start -D
```
...
打开 fustord 管理界面，进入 **"Connected datacasts"** 面板。此时你应该能看到已上线的 4 datacastcast。

### 3.1 批量注入 datacast 配置
无需逐台登录 datacast 节点，在 UI 上点击 **"Config"** 按钮即可完成下发：

1.  **配置 datacast-1 datacastcast-2**：
    粘贴以下 YAML 并点击 **"Push Config"**。datacast 将在下一次心跳（约 3s 内）接收配置并自动热加载：
    ```yaml
    datacast_iddatacastcast-1  # 对应节点 ID
    sources:
      src-1:
        driver: fs
    ...
    pipes:
      pipe-fs-1:  # datacast-1 datacastcast-2 必须使用相同的 pipe_id
        source: src-1
        sender: fustord-main
    ```

2.  **配置 datacast-3 datacastcast-4**：
...
### 5.1 远程版本升级 (Upgrade)
当 Fustor 发布新版本时：
1.  在 UI 的 **Connected datacasts** 列表中找到目datacastcast。
2.  点击 **"Upgrade"** 按钮。
3.  输入目标版本号（例如 `0.9.1`）。
...
3.  点击该 datacast 的 **"Reload"** 按钮发送 SIGHUP 信datacastcast 将重新加载配置，当前同步 Session 不会中断。
