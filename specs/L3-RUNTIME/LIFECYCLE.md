...
3. **Acknowledgment**: Fusion 在响应中返回最终决定的 `session_timeout_seconds`，sensord **必须** 采纳此值作为心跳间隔的基准。
...
当 Fusion 收到 `audit_start` 但超过一定时间（建议为 `2 * audit_interval_sec`）未收到对应的 `audit_end` 时，应自动关闭该审计窗口并记录警告日志。这可以防止 sensord 崩溃在 Audit 中间时，Fusion 的审计窗口永远开启。
...
Fusion can issue commands to the sensord via the Heartbeat response channel.
