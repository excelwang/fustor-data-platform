...
- **RESILIENCE**: Agent process runs indefinitely; self-healing on all non-fatal errors
...
- **AGENT_SURVIVAL**: Agent 进程启动后，**绝不因**业务逻辑错误、数据损坏或插件故障而终止。它本质上是一个不朽守护进程，唯一任务是维持到 Fusion 的生命线。
...
- **UMBILICAL_CORD**: 只要控制面（心跳、任务分发、状态上报）保持完整，系统就具备无限的修复潜力：
  ...
  2. **热升级 (Hot Upgrades)**: Agent 接收软件版本更新，实现在线自置换。
...
### Hot Upgrade
- **单点触发，进程重启**: 对于多连接 (Multi-Pipe) 的 Agent 进程，升级指令必须具备**精准下发 (Targeted)** 能力。无论多少个 Session 活跃，Fusion 仅触发其中一个，由 Agent 完成全局自置换。
- **透明恢复**: 升级后，Agent 自动恢复所有业务连接。

### Config Hot-Reload
- **全局生效 (Process-Wide)**: 新配置必须在 Agent 所有组件中同步生效。
...
- **UPTIME**: `fustor-agent` 进程运行时间以**月**计量，即使 `fustor-source-fs` 每日重启。
- **ZOMBIE_RECOVERY**: Fusion 可通过控制面诊断"僵尸 Agent"（数据面卡死）并远程发出 `kill -9` + `restart` 命令。
- **ZERO_TOUCH**: 无需 SSH。如果错误正则崩溃了 scanner，Fusion 可推送修正正则，Agent Shell 用新配置重启 scanner。
...
| Term | Definition |
|------|------------|
| Agent | 部署在数据节点上的自主传感器进程 |
| Fusion | 中央聚合与协调服务 |
| Pipe | Agent/Fusion 之间的逻辑数据管道 |
