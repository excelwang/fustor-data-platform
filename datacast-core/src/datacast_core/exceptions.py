...
class ConfigError(DatacastException):
...
class NotFoundError(DatacastException):
...
class ViewNotReadyError(DatacastException):
    """Raised when a view is not ready (no leader or initial snapshot in progress)."""
    pass


class ConflictError(DatacastException):
...
class DriverError(DatacastException):
...
class StateConflictError(DatacastException):
...
class ValidationError(DatacastException):
...
class SessionObsoletedError(DatacastException):
...
class fustordConnectionError(DatacastException):
