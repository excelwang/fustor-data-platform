...
class ViewDriver(ABC):
    ...
    This is the fustord-side counterpart to SourceDriver (datacast-side).
    ...
    async def on_session_start(self, **kwargs):
        """Called when a new datacast session starts."""
        pass
    
    async def on_session_close(self, **kwargs):
        """Called when an datacast session terminates."""
        pass
...
class SenderDriver(ABC):
    ...
    Defines the contract for drivers that receive data from the Fudatacast core
...
class SourceDriver(ABC):
    ...
    Defines the contract for drivers that produce data for the Fudatacast core.
    """
    ...
    # If False, the datacast will skip the discovery step and consider the schema valid.
    ...
    async def create_datacast_user(cls, **kwargs) -> Tuple[bool, str]:
        """
        Optional: Creates a datacast user for the source service.
        """
        return (True, "datacast user creation not implemented for this driver.")