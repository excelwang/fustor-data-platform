...
class BasePipeManager(BaseInstanceService[T], Generic[T], ABC):
    """
    Base class for Pipe Managers in datacast and fustord.
    ...
    async def start_all(self, ids: List[str]) -> Dict[str, Any]:
        ...
        for pid, result in zip(ids, results):
            if isinstance(result, Exception):
                ...
                normalized_results.append(StartResult(datacast_pipe_id=pid, success=False, error=str(result)))
            elif isinstance(result, StartResult):
                normalized_results.append(result)
            else:
                # Should not happen if start_one returns StartResult
                normalized_results.append(StartResult(datacast_pipe_id=pid, success=False, error="Unknown result type"))
        ...
        for r in normalized_results:
            if not r.success:
                self.logger.error(f"  - {r.datacast_pipe_id}: {r.error}")
