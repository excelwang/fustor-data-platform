...
authority to eliminate clock drift issues across distributed datacasts.
"""
import threading
import time
...
class LogicalClock:
    ...
    def __init__(self):
        ...
        # datacast-side (multi-threaded scanning) and fustord-side (asyncio) contexts.
    ...
    def update(self, observed_mtime: float, can_sample_skew: bool = True) -> float:
        ...
        # This makes the system immune to datacast local clock errors (Faketime/NTP drift).
        ...
        with self._lock:
            ...
            try:
                ...
                if not hasattr(self, '_logger'):
                    import logging
                    self._logger = logging.getLogger("datacast_core.clock")
