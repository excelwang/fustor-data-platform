...
class DatacastPipe(ABC):
    """
    Abstract base class for all Fustor Pipes (datacast or fustord).
    
    A DatacastPipe orchestrates:
    - Session lifecycle management
    - Handler invocation (Source/Sender or Receiver/View)
    - Heartbeat and timeout handling
    - Error recovery
    
    datacast DatacastPipe: Source -> Sender
    fustord DatacastPipe: Receiver -> View(s)
    """
    ...
    def __str__(self) -> str:
        return f"DatacastPipe({self.id}, state={self.state.name})"
