...
This module provides the logic to fallback to a realtime datacast command
...
# to prevent overwhelming datacasts or network.
...
async def on_command_fallback(view_id: str, params: Dict[str, Any], pipe_manager: "PipeManager") -> Dict[str, Any]:
    """
    Execute a remote scan on the datacast when local view query fails.
    """
    ...
    try:
        ...
        return {
            ...
            "metadata": {
                ...
                "timestamp": time.time(),
                "datacast_id": best_result.get("datacast_id")
            }
        }
    except Exception as e:
