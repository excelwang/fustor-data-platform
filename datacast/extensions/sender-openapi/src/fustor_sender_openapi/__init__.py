"""
Fudatacast sender driver for OpenAPI endpoints. (Class-based refactoring)
"""
import httpx
import logging
...
from datacast_core.exceptions import DriverError, SessionObsoletedError
from datacast_core.models.config import SenderConfig, ApiKeyCredential, PasswdCredential
from datacast_core.event import EventBase
from datacast_core.utils.retry import retry

logger = logging.getLogger("datacast.driver.openapi")
...
class OpenApiDriver(Sender):
        ...
        **kwargs
        ...
        if servers and isinstance(servers, list) and len(servers) > 0:
            ...
            if isinstance(first_server, dict):
                batch_endpoint_path = first_server.get("x-datacast-ingest-batch-endpoint")        
        
        # If x-datacast-ingest-batch-endpoint is not defined in OpenAPI spec, extract from available endpoints
        if not batch_endpoint_path:
            ...
            if not batch_endpoint_path:
                logger.debug(f"Sender endpoint {self.endpoint} spec does not define 'x-datacast-ingest-batch-endpoint' and no suitable batch endpoint found in spec. Using default path '/batch'.")
                batch_endpoint_path = "/batch"
            else:
                logger.debug(f"Sender endpoint {self.endpoint} spec does not define 'x-datacast-ingest-batch-endpoint', using discovered path '{batch_endpoint_path}' from available endpoints.")
    ...
    async def create_session(
        ...
        if servers and isinstance(servers, list) and len(servers) > 0:
            first_server = servers[0]
            if isinstance(first_server, dict):
                session_path = first_server.get("x-datacast-open-session-endpoint")
        ...
        if not session_path:
            ...
            if not session_path:
                logger.debug(f"Sender endpoint {self.endpoint} spec does not define 'x-datacast-open-session-endpoint' and no suitable session endpoint found in spec. Using default path '/events/session'.")
                session_path = "/events/session"
            else:
                logger.debug(f"Sender endpoint {self.endpoint} spec does not define 'x-datacast-open-session-endpoint', using discovered path '{session_path}' from available endpoints.")
    ...
    async def heartbeat(self) -> Dict[str, Any]:
        ...
        if servers and isinstance(servers, list) and len(servers) > 0:
            first_server = servers[0]
            if isinstance(first_server, dict):
                heartbeat_path = first_server.get("x-datacast-heartbeat-endpoint")
        ...
        if not heartbeat_path:
            ...
            if not heartbeat_path:
                ...
                logger.debug(f"Sender endpoint {self.endpoint} spec does not define 'x-datacast-heartbeat-endpoint', using discovered path '{heartbeat_path}' from available endpoints.")
                server_url = spec.get("servers", [{}])[0].get("url", "")
                base_url = urljoin(self.endpoint, server_url)
    ...
    async def check_privileges(cls, **kwargs) -> Tuple[bool, str]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                servers = spec.get("servers", [])
                if servers:
                    status_path = servers[0].get("x-datacast-status-endpoint")

                # If x-datacast-status-endpoint is not defined in OpenAPI spec, extract from available endpoints
                if not status_path:
                    ...
                    if not status_path:
                        return (False, "无法在 OpenAPI 规范中找到 'x-datacast-status-endpoint' 或其他可用的状态端点，无法执行权限检查。")
