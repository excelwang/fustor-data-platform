...
from datacast_core.models.config import AppConfig, PipeConfig, FieldMapping
...
from datacast_sdk.interfaces import PipeConfigServiceInterface # Import the interface
from datacast.config.unified import DatacastConfig, DatacastPipeConfig # New Unified YAML loader
...
class PipeConfigService(BaseConfigService[PipeConfig], PipeConfigServiceInterface): # Inherit from the interface
    ...
    def _convert_yaml_to_model(self, y_cfg: DatacastPipeConfig) -> PipeConfig:
    ...
    def get_config(self, id: str) -> Optional[PipeConfig]:
        ...
        # 1. Try YAML first
        yaml_config = DatacastConfig.get_pipe(id)
        if yaml_config:
    ...
    def list_configs(self) -> Dict[str, PipeConfig]:
        ...
        yaml_configs = DatacastConfig.get_all_pipes()
