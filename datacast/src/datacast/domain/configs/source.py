# src/datacast/services/configs/source.py

import logging
from typing import Dict, Optional, List, Any


from datacast_core.models.config import AppConfig, SourceConfig
...
from datacast_sdk.interfaces import SourceConfigServiceInterface # Import the interface
...
class SourceConfigService(BaseConfigService[SourceConfig], SourceConfigServiceInterface): # Inherit from the interface
    ...
    def _add_config_to_app(self, id: str, config: SourceConfig):
        from datacast.config.unified import DatacastConfig
        DatacastConfig.add_source(id, config)

    def _delete_config_from_app(self, id: str) -> SourceConfig:
        from datacast.config.unified import DatacastConfig
        conf = DatacastConfig.get_source(id)
        DatacastConfig.delete_source(id)
        return conf
    ...
    async def cleanup_obsolete_configs(self) -> List[str]:
        ...
        """
        from datacast.config.unified import DatacastConfig
        all_pipe_configs = DatacastConfig.get_all_pipes().values()
        ...
        async with config_lock:
            for an_id in obsolete_ids:
                ...
                from datacast.config.unified import DatacastConfig
                if DatacastConfig.get_source(an_id):
        ...
        for source_id, config in self.list_configs().items():
            ...
            if not config.disabled and not schema_cache.is_schema_valid(source_id):
                ...
                logger.warning(
                    ...
                    f"Please run 'datacast discover-schema' to re-validate."
                )
    ...
    async def discover_and_cache_fields(self, source_id: str, admin_user: str, admin_password: str):
        ...
        """
        from datacast_core.models.config import PasswdCredential
        from ..drivers.source_driver import SourceDriverService
