#!/usr/bin/env python
"""
Fustor datacast CLI - Simplified WireGuard-style interface.

Commands:
  datacast start [configs...]  # Start pipe(s), default.yaml if no args
  datacast stop [configs...]   # Stop pipe(s)
  datacast list                # List running pipes
  datacast status [pipe]       # Show status
"""
import click
import asyncio
import signal
...
from datacast_core.common import setup_logging, get_fustor_home_dir
from .config.unified import DatacastConfig

# Define common paths
HOME_FUSTOR_DIR = get_fustor_home_dir()
LOG_DIR = os.path.join(HOME_FUSTOR_DIR, "logs")
AGENT_LOG_FILE = os.path.join(LOG_DIR, "datacast.log")
PID_FILE = os.path.join(HOME_FUSTOR_DIR, "datacast.pid")


def _is_running():
    """Check if datacast daemon is running."""
...
def start(configs, daemon, verbose, no_console_log):
    ...
    Examples:
        datacast start                    # Start all from default.yaml
        datacast start research-sync      # Start single pipe
        datacast start pipe-a pipe-b      # Start multiple pipes
        datacast start custom.yaml        # Start from managed file
    """
    ...
    if verbose:
        ...
        DatacastConfig.ensure_loaded()
        log_level = DatacastConfig.logging.level if hasattr(DatacastConfig, "logging") else "INFO"

    setup_logging(
        log_file_path=AGENT_LOG_FILE,
        base_logger_name="datacast",
    ...
    if daemon:
        pid = _is_running()
        if pid:
            click.echo(f"datacast already running with PID: {pid}")
            return
        
        click.echo("Starting datacast in background...")
        ...
        try:
            ...
            click.echo(f"datacast started with PID: {proc.pid}")
    ...
    try:
        ...
        from .runner import run_datacast
        
        # Convert configs tuple to list, empty means use default.yaml
        config_list = list(configs) if configs else None
        
        click.echo(f"datacast starting... Logs: {AGENT_LOG_FILE}")
        asyncio.run(run_datacast(config_list))

    except KeyboardInterrupt:
        click.echo("\ndatacast shutting down...")
    except Exception as e:
...
def stop(configs):
    """
    Stop pipe(s) or the entire datacast.
    
    If no configs specified, stops all pipes and the daemon.
    
    Examples:
        datacast stop                     # Stop everything
        datacast stop research-sync       # Stop single pipe (hot)
    """
    pid = _is_running()
    ...
    if not configs:
        # Stop entire daemon
        if not pid:
            click.echo("datacast is not running.")
            return
        
        click.echo(f"Stopping datacast (PID: {pid})...")
        try:
...
def list_pipes():
    ...
    DatacastConfig.ensure_loaded()
    pipes = DatacastConfig.get_all_pipes()
    ...
    for pid, pcfg in pipes.items():
        source = DatacastConfig.get_source(pcfg.source)
...
def status(pipe_id):
    """Show status of datacast or specific pipe."""
    pid = _is_running()
    
    if not pid:
        click.echo("datacast Status: " + click.style("STOPPED", fg="red"))
    else:
        click.echo("datacast Status: " + click.style(f"RUNNING (PID: {pid})", fg="green"))
    
    if pipe_id:
        DatacastConfig.ensure_loaded()
        pipe = DatacastConfig.get_pipe(pipe_id)
        if not pipe:
        ...
        source = DatacastConfig.get_source(pipe.source)
