# src/datacast/app.py
"""
datacast Application - Main orchestrator using new unified pipe config V2.
"""
import asyncio
import json
import logging
...
from datacast_core.models.config import AppConfig

from .config.unified import DatacastConfig, DatacastPipeConfig
...
STATE_FILE_PATH = os.path.join(HOME_FUSTOR_DIR, "datacast-state.json")
...
class App:
    ...
    Refactored to use unified DatacastConfigLoader V2 (Dict-based).
    """
    ...
    def __init__(self, config_list: Optional[List[str]] = None):
        ...
        """
        self.logger = logging.getLogger("datacast")
        ...
        self.config_signature = DatacastConfig.get_config_signature()
        
        # Initialize Config Services (adapter for PipeManager)
        # Note: In future, PipeManager might use DatacastConfig directly
    ...
    def _resolve_target_pipes(self, config_list: Optional[List[str]]) -> List[str]:
        ...
        if config_list is None:
            ...
            all_default = DatacastConfig.get_default_pipes()
            targets = []
    ...
    def _validate_runtime_uniqueness(self, pipe_ids: List[str]):
        ...
        for pid in pipe_ids:
            resolved = DatacastConfig.resolve_pipe_refs(pid)
            if not resolved:
                continue
            p_cfg = resolved['pipe']
            ...
            if pair in seen_pairs:
                error_msg = (
                    ...
                    f"datacast will not start conflicting pipes to prevent data corruption."
                )
    ...
    async def reload_config(self):
        ...
        new_signature = DatacastConfig.get_config_signature()
        ...
        diff = DatacastConfig.get_diff(current_running_ids)
