...
from datacast.config.unified import DatacastConfigLoader, DatacastConfig
from datacast_core.models.config import SourceConfig, SenderConfig, GlobalLoggingConfig


logger = logging.getLogger("datacast.validator")

class ConfigValidator:
    """
    Validates datacast configuration for consistency and completeness.
    """
    
    def __init__(self, loader: DatacastConfigLoader = DatacastConfig):
        self.loader = loader
    ...
    def validate_config(self, config_dict: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        Leverages UnifiedDatacastConfig for Pydantic structural validation and 
        ...
        try:
            ...
            # Pydantic handles structural validation. defaults (like datacast_id being Optional) are handled here.
            unified_cfg = UnifiedDatacastConfig.model_validate(config_dict)
            
            # 2. Build temp loader for semantic check
            temp_loader = DatacastConfigLoader(config_dir=None)
