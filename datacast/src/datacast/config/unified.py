# datacast/src/datacast/config/unified.py
"""
Unified Configuration Loader for datacast.

Config directory: $FUSTOR_HOME/datacast-config/
...
from datacast_core.models.config import SourceConfig, SenderConfig, GlobalLoggingConfig, FieldMapping
...
class DatacastPipeConfig(BaseModel):
    """Configuration for a single datacast pipe."""
...
class UnifiedDatacastConfig(BaseModel):
    ...
    pipes: Dict[str, DatacastPipeConfig] = {}
    
    # Tuning parameters
    fs_scan_workers: int = Field(default=4, description="Default concurrency for FS scans")


class DatacastConfigLoader:
    """
    Loads and merges all config files from datacast-config/ directory.
    ...
    def __init__(self, config_dir: Optional[Path] = None):
        if config_dir is None:
            ...
            config_dir = home / "datacast-config"
        ...
        self.datacast_id: Optional[str] = None

        # Merged namespace
        self._sources: Dict[str, SourceConfig] = {}
        self._senders: Dict[str, SenderConfig] = {}
        self._pipes: Dict[str, DatacastPipeConfig] = {}
    ...
    def _load_file(self, path: Path) -> None:
        ...
        try:
            ...
            if "datacast_id" in data:
                aid = str(data["datacast_id"]).strip()
                if self.datacast_id and self.datacast_id != aid:
                    logger.warning(f"datacast ID redefined in {path}: was '{self.datacast_id}', now '{aid}'")
                self.datacast_id = aid
            ...
            for pipe_id, pipe_data in data.get("pipes", {}).items():
                ...
                self._pipes[pipe_id] = DatacastPipeConfig(**pipe_data)
    ...
    def get_pipe(self, pipe_id: str) -> Optional[DatacastPipeConfig]:
        self.ensure_loaded()
    ...
    def get_all_pipes(self) -> Dict[str, DatacastPipeConfig]:
        self.ensure_loaded()
        return self._pipes.copy()
    
    def get_pipes_from_file(self, filename: str) -> Dict[str, DatacastPipeConfig]:
    ...
    def get_default_pipes(self) -> Dict[str, DatacastPipeConfig]:
        """Get pipes from default.yaml."""
        return self.get_pipes_from_file("default.yaml")
    
    def get_enabled_pipes(self) -> Dict[str, DatacastPipeConfig]:
        """
        Get all enabled pipes.
        In datacast, a pipe is enabled if its source is enabled.
        """
        self.ensure_loaded()
        enabled = {}
...
# Global instance
DatacastConfig = DatacastConfigLoader()
