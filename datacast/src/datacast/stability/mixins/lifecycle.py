...
from datacast_core.exceptions import SessionObsoletedError, fustordConnectionError
from datacast.config.unified import get_outbound_ip

if TYPE_CHECKING:
    from ..pipe import DatacastPipe

logger = logging.getLogger("datacast.pipe.lifecycle")

class PipeLifecycleMixin:
    """
    Mixin for DatacastPipe lifecycle and control loop management.
    Expected to be mixed into DatacastPipe class.
    """
    
    def _calculate_backoff(self: "DatacastPipe", consecutive_errors: int) -> float:
    ...
    def _handle_control_error(self: "DatacastPipe", error: Exception, loop_name: str) -> float:
    ...
    def _handle_data_error(self: "DatacastPipe", error: Exception, loop_name: str) -> float:
    ...
    def _common_error_handler(self: "DatacastPipe", error_count: int, error: Exception, loop_name: str, is_control: bool) -> float:
    ...
    async def _run_control_loop(self: "DatacastPipe") -> None:
        ...
        while self.is_running():
            ...
            try:
                ...
                if not self.has_active_session():
                    ...
                    try:
                        # Resolve datacast ID: prefer configured id, fallback to dynamic IP
                        from datacast.config.unified import DatacastConfig
                        datacast_id = DatacastConfig.datacast_id
                        
                        if not datacast_id:
                            # Resolve dynamic datacast ID (IP) based on Sender endpoint
                            fustord_uri = getattr(self.sender_handler, "endpoint", "8.8.8.8")
                            if not isinstance(fustord_uri, str):
                                fustord_uri = "8.8.8.8"
                            datacast_id = get_outbound_ip(fustord_uri)
                        
                        self.task_id = f"{datacast_id}:{self.id}"
