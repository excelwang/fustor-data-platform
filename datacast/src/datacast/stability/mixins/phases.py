...
logger = logging.getLogger("datacast.pipe.phases")

async def run_snapshot_sync(pipe: "DatacastPipe") -> None:
        ...
        async for event in snapshot_iter:
            ...
            if len(batch) >= pipe.batch_size:
                ...
                if success:
                    ...
                    get_metrics().counter("fustor.datacast.events_pushed", len(batch), {"pipe": pipe.id, "phase": "snapshot"})
                    batch = []
                else:
...
async def run_bus_message_sync(pipe: "DatacastPipe") -> None:
    ...
    try:
        while pipe.is_running() or (pipe.state & PipeState.RECONNECTING):
            ...
            if success:
                ...
                get_metrics().counter("fustor.datacast.events_pushed", len(events), {"pipe": pipe.id, "phase": "realtime_bus"})
...
async def run_audit_sync(pipe: "DatacastPipe") -> None:
    ...
    try:
        ...
        async for item in audit_iter:
            ...
            if len(batch) >= pipe.batch_size:
                ...
                get_metrics().counter("fustor.datacast.events_pushed", len(batch), {"pipe": pipe.id, "phase": "audit"})
                batch = []
...
async def run_sentinel_check(pipe: "DatacastPipe") -> None:
    ...
    try:
        ...
        if results:
            ...
            if success:
                ...
                get_metrics().counter("fustor.datacast.sentinel_checks", 1, {"pipe": pipe.id})
            else:
