# datacast/src/datacast/runtime/pipe/heartbeat.py
import asyncio
import logging
...
class PipeHeartbeatMixin:
    """
    Mixin for DatacastPipe heartbeat and command management.
    """
    
    def map_batch(self: "DatacastPipe", events: List[Any]) -> List[Any]:
        """Map events using the configured field mapper."""
        return self._mapper.map_batch(events)

    async def _handle_commands(self: "DatacastPipe", commands: List[Dict[str, Any]]) -> None:
    ...
    def _load_mgmt_processor(self: "DatacastPipe") -> Optional[Any]:
        """Dynamically load the L3 management processor if available."""
        # Use class-level cache from DatacastPipe (self.__class__)
        cls = self.__class__
        ...
        try:
            eps = entry_points(group="datacast.command_processors")
            for ep in eps:
                try:
    ...
    def _build_datacast_status(self: "DatacastPipe") -> Dict[str, Any]:
        """Build datacast status dict for heartbeat reporting."""
        datacast_id = None
        if self.task_id and ":" in self.task_id:
            datacast_id = self.task_id.split(":")[0]
    ...
    async def _update_role_from_response(self: "DatacastPipe", response: Dict[str, Any]) -> None:
        ...
        if new_role and new_role != self.current_role:
            get_metrics().gauge("fustor.datacast.role", 1 if new_role == "leader" else 0, {"pipe": self.id, "role": new_role})
    ...
    async def _handle_role_change(self: "DatacastPipe", new_role: str) -> None:
    ...
    async def on_session_created(self: "DatacastPipe", session_id: str, **kwargs) -> None:
    ...
    async def on_session_closed(self: "DatacastPipe", session_id: str) -> None:
    ...
    async def _run_heartbeat_loop(self: "DatacastPipe") -> None:
        ...
        while self.has_active_session():
            try:
                ...
                response = await self.sender_handler.send_heartbeat(
                    ...
                    datacast_status=self._build_datacast_status(),
                )
