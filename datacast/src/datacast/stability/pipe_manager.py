...
# PipeRuntime is now always DatacastPipe
PipeRuntime = DatacastPipe

from datacast.domain.source_handler_adapter import SourceHandlerAdapter
from datacast.stability.sender_adapter import SenderHandlerAdapter
from datacast.config.unified import DatacastConfig

logger = logging.getLogger("datacast")

from datacast_sdk.interfaces import PipeManagerInterface # Import the interface
...
class PipeManager(BasePipeManager, PipeManagerInterface): # Inherit from base and interface
    ...
    async def start_one(self, id: str, raise_on_error: bool = False) -> StartResult:
        ...
        if self.get_instance(id):
            ...
            return StartResult(datacast_pipe_id=id, success=True, skipped=True)

        pipe_config = DatacastConfig.get_pipe(id)
        if not pipe_config:
            error_msg = f"Pipe config '{id}' not found."
            self.logger.error(error_msg)
            if raise_on_error:
                raise NotFoundError(error_msg)
            return StartResult(datacast_pipe_id=id, success=False, error=error_msg)
        ...
        source_config = DatacastConfig.get_source(pipe_config.source)
        ...
        sender_config = DatacastConfig.get_sender(pipe_config.sender)
        ...
        try:
            ...
            return StartResult(datacast_pipe_id=id, success=False, error=friendly_msg)
    ...
    async def mark_dependent_pipes_outdated(self, dependency_type: str, dependency_id: str, reason_info: str, updates: Optional[Dict[str, Any]] = None):
        ...
        for inst in self.list_instances():
            pipe_config = DatacastConfig.get_pipe(inst.id)
            if not pipe_config:
                continue
        ...
        for pipe_instance in affected_pipes:
            # Use PipeState which is what DatacastPipe uses internally
    ...
    async def start_all_enabled(self) -> Dict[str, Any]:
        ...
        """
        all_pipe_configs = DatacastConfig.get_all_pipes()
