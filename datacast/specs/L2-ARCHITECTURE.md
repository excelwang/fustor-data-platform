# L2: [Datacast] System Architecture

> This document defines the high-level component structure of **Datacast**.
...
```mermaid
graph TD
    subgraph "Datacast Domain Layer (核心业务 - SDP)"
        DatacastSource["Source Drivers (FS/SQL/etc.)"]
        DatacastCore["Datacast-core (ABCs/Events)"]
        SDP["Datacast Data Protocol"]
    end

    subgraph "Datacast Stability Layer (稳定性层 - SCP)"
        DatacastPipe["DatacastPipe (Lifecycle/Heartbeat)"]
        EventBus["EventBus (Memory Buffer)"]
        Sender["Sender Drivers (HTTP/gRPC)"]
        BaseMgr["Datacast-core (BasePipeManager)"]
        SCP["Datacast Control Protocol"]
    end

    %% Dependencies
    DatacastPipe <==>|SCP (Umbilical Cord)| Consumer["External Consumer (Aggregator)"]
    DatacastPipe ---|Spawns/Monitors| DatacastSource
    DatacastSource ---|SDP (Data Contract)| EventBus
    EventBus ---|Drained by| DatacastPipe
    DatacastPipe ---|Uses| Sender
```

### Layer Definitions

**Datacast** 采用 **"下沉稳定性，上行扩展性"** 的三层垂直模型：
...
```mermaid
graph TD
    subgraph "Datacast (Standalone Project)"
        Core["Datacast-core (Foundation)"]
        Daemon["Datacast (The Shell)"]
        SrcFS["Datacast-source-fs (Scanner Driver)"]
        SendHTTP["Datacast-sender-http (Transport)"]
        SchemaFS["Datacast-schema-fs (FS Data Contract)"]
    end
...
|------------------|-------------------------|------|
| `Datacast-core` | `Datacast-core` | Foundation ABCs & Models |
| `Datacast` | `Datacast` | Process Guardian (SCP/SDP Orchestrator) |
| `fustor-source-fs` | `Datacast-source-fs` | FS Scanner Driver |
| `fustor-schema-fs` | `Datacast-schema-fs` | FS Event Schema (SDP Implementation) |
| `fustor-sender-*` | `Datacast-sender-*` | Network Transporters |
...
#### Datacast 侧数据流向 (SDP Transmission)
...
│   ───────────                                                                        │
│                                                                                      │
│   Source-A ──┬── DatacastPipe-1 ──┬── Sender (HTTP) ──▶ Consumer A (SDP)              │
│   Source-B ──┘                   └── Sender (HTTP) ──▶ Consumer B (SDP)              │
│                                                                                      │
│   约束: <source, sender> 组合唯一 (由 DatacastPipe 负责生命周期维护)                     │
│                                                                                      │
└─────────────────────────────────────────────────────────────────────────────────────┘
```
...
│                                                                                      │
│   ┌──────────────┐         ┌─────────────────┐         ┌──────────────┐             │
│   │  Local Watch │────────▶│    EventBus     │────────▶│  DatacastPipe   │──▶ Consumer  │
...
Session 是 **Datacast** 与 **Consumer** 之间通过 **SCP** 建立的业务租赁关系。
...
**Component**: 配置管理子系统。
- **Input**: `$DATACAST_HOME` 下的 YAML 文件 (sources, senders, pipes)
- **Output**: 类型安全的配置对象 (Pydantic Models)

**Root Path**: `$DATACAST_HOME` (default: `/etc/Datacast`)
...
Datacast 期望消费端实现以下最小符合性接口：

| 功能 | 协议 | 方向 |
|------|------|------|
| Session 创建/销毁 | SCP | Datacast → Consumer |
| 数据事件推送 | SDP | Datacast → Consumer |
| 心跳 + 指令下发 | SCP | 双向 |
| 远程管理 (升级/重载) | SCP | Consumer → Datacast |
...
Datacast 采用 **Peer-to-Peer** 协作模型，而非简单的被动执行模型：

*   **租用模型 (Renting)**: Datacast 视外部系统为“信道提供方”。当需要发布数据时，通过 SCP 启动一个租约进程（Session）。
*   **生存隔离**: 底层 `Stability Layer` 必须在逻辑上隔离各 Session。
*   **控制与数据解耦**: SCP 维持生存，SDP 维持契约。两者在协议层面互不干扰。

### 9.1 Unified Renting Model (Task Types)

Datacast 作为租户，通过 Stability Layer (SCP) 接收两种类型的任务租约：

#### 1. Broadcast Task (内容补偿)
- **场景**: `scan` (On-Demand Find)
- **行为**: Aggregator 广播给同一 View 的所有 Datacast。
- **Datacast 职责**: 执行只读扫描，通过 SDP 回传数据。

#### 2. Targeted Task (状态变更)
- **场景**: `upgrade`, `reload`, `stop`
- **行为**: Aggregator 单播给特定 task_id 的 Datacast。
- **Datacast 职责**: 执行自身状态变更或重启。
