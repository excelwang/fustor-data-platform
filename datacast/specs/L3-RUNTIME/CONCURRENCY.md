# L3: [Datacast] Concurrency & Threading Model
...
- **SINGLE_EVENT_LOOP**: Datacast core logic MUST run on a single `asyncio` event loop.
...
To bridge synchronous Blocking I/O (e.g., `os.scandir`, `inotify` reads) with the Asyncio world, Datacast uses the **Thread Bridge Pattern**:
