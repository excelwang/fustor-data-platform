# L3: [workflow] [Datacast] Runtime Lifecycle & Self-Healing
...
## [model] Pipe_State_Machine_Model

每一个数据流动管道（DatacastPipe）在 **Datacast** 内部拥有独立的生命周期：
...
## [mechanism] Heartbeat_and_Survival_Mechanism

**Datacast** 与消费者之间的 SCP 心跳是生存决策的核心机制：

### 2.1 核心不变量 (Invariant)
**心跳循环 (Heartbeat Tunnel) 必须永续运行**。只要进程存活且任务启用，心跳循环永远不应因业务 Task (如 Audit/Scan) 的崩溃而终止。

### 2.2 脐带功能 (Umbilical Functions)
1.  **生存权置换**: 消费者在心跳响应中下发指令。若心跳中断超过 `timeout_seconds`，Datacast **必须** 停止该 Session 下的数据推送并进入 `RECONNECTING` 状态。
...
2. **Acknowledgment**: Aggregator 在响应中返回最终决定的 `session_timeout_seconds`，Datacast **必须** 采纳此值作为心跳间隔的基准。
...
| `CancelledError` (- STOPPING) | **Continue** | 单个 Task (如 Snapshot) 被取消，但 DatacastPipe 仍需运行 (见 §2)。 |
...
| 状态 | 含义 | 用途 |
|------|------|------|
| `PipeState.RUNNING` | Pipe 健康、已连接、正在处理事件（包括 SYNCING 等子状态） | 监控 / CLI 状态展示 |

---

## [context] Audit_Cache_Context

`Audit Context` (mtime cache) 是 Datacast 进程内的物理缓存，用于“True Silence”优化。

- **不持久化**: 此缓存仅存在于内存，Datacast 重启后首轮执行物理全量审计是设计的鲁棒性边界。
- **强制失效**: 当 Session 重建或发生角色晋升时，Audit Context 必须被显式清空，以防止不同会话间的时间轴由于 Clock Drift 出现空隙。

---

## [mechanism] Hot_Watch_Set_Management
...
- **漂移计算**: 计算 NFS Server 与 Datacast 本地时间的 `drift`，并在设置 Watch 时修正时间戳。
...
## [strategy] Fault_Isolation_and_Self_Healing

### 5.1 核心原则
- **控制面优先**: 即使数据驱动（Data Plane）抛出 `RuntimeError`，**Datacast** 的 `Stability Layer` 必须接管该异常，将 Pipe 切回 `RECONNECTING` 或 `INITIALIZING` 状态，尝试重建。
...
| **进程级** (OOM/Crash) | 影响整个 Datacast 容器 | **Process Guard**: 依赖外部宿主环境 (systemd/k8s) 重新启动。 |
...
| 组件 | 层面 | 说明 |
|------|------|------|
| **Source** | Datacast | 数据源驱动（如 FSDriver） |
| **DatacastPipe** | Datacast | 代理管道 |
| **Sender** | Datacast | 上行发送通道 |

#### 级联故障规则 (Cascading Failure Rules)

- **Pipe 失败不影响其他 Pipe**: 每个 Pipe 独立运行，某个 Pipe 崩溃不影响同进程内的其他 Pipe
- **Source/Sender 失败 → 关联 DatacastPipe 全部失败**: Source 或 Sender 是共享组件，其故障影响所有依赖它的 DatacastPipe
...
## [interface] Command_Execution_Interface

**Rationale**: Enable the Control Plane to execute high-priority repairs or diagnostics even when the Data Plane is busy.

```python
# Command dispatch signature
async def execute_remote_command(pipe_id: str, command: Command): ...
```

Datacast 必须能够在任何状态下（包含 Follower 或 初始化中）响应来自 Consumer 的突发控制任务：
...
## [observability] System_Health_Observability
