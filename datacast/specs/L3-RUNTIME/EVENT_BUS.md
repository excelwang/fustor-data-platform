# L3: [pattern] [Datacast] MemoryEventBus (Async Dispatch)
...
为了将 **Domain Layer** (扫描产生的事件) 与 **Stability Layer** (网络推送逻辑) 完全解耦，**Datacast** 内部核心使用了一个有界环形缓冲区 `MemoryEventBus`。
...
- **规则**: `EventBus` 的生命周期随 **Datacast** 进程启动而建立，随最后一条 `DatacastPipe` 销毁而释放资源。
