# datacast/tests/runtime/test_control_data_isolation.py
"""
...
class TestControlDataIsolation:
    ...
    @pytest.fixture
    def datacast_pipe(self, mock_source, mock_sender, pipe_config, caplog):
        import logging
    ...
    @pytest.mark.asyncio
    async def test_control_loop_not_blocked_by_slow_snapshot(self, datacast_pipe, mock_sender, mock_source):
        ...
        for _ in range(50): # 5 seconds
            if datacast_pipe._snapshot_task and not datacast_pipe._snapshot_task.done():
                found_task = True
                break
            await asyncio.sleep(0.1)
            
        if not found_task:
            debug_info = f"Role: {datacast_pipe.current_role}, Realtime: {datacast_pipe.is_realtime_ready}, InitialDone: {datacast_pipe._initial_snapshot_done}"
            # Log failure details
        ...
        assert datacast_pipe._snapshot_task is not None
        assert not datacast_pipe._snapshot_task.done()
        ...
        assert datacast_pipe.current_role == "follower"
        ...
        if datacast_pipe._snapshot_task:
             assert datacast_pipe._snapshot_task.done() or datacast_pipe._snapshot_task.cancelled()
    ...
    @pytest.mark.asyncio
    async def test_independent_error_counters(self, datacast_pipe, mock_sender, mock_source):
        ...
        datacast_pipe.audit_interval_sec = 0.1
        ...
        # Check counters
        assert datacast_pipe._data_errors > 0, "Should have accumulated data errors"
        assert datacast_pipe._control_errors == 0, "Control errors should remain 0"
        ...
        # Let's verify the code change we made to datacast_pipe.py...
