# datacast/tests/runtime/test_pipe_terminal_error.py
"""
U1: max_consecutive_errors 触发后 Pipe 的行为。

被测代码: datacast/src/datacast/runtime/datacast_pipe.py → _handle_loop_error() + _run_control_loop()
...
def pipe_with_low_max_errors():
    ...
    pipe = DatacastPipe("test-pipe", config, src, snd)
    return pipe, src, snd
