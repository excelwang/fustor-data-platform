...
from datacast_core.pipe.pipe import PipeState
...
def mock_pipe():
    ...
    source_h = MagicMock()
    sender_h = AsyncMock()
    # DatacastPipe will load CommandProcessor if installed (which it is since we ran uv sync)
...
async def test_handle_command_stop_via_mgmt(mock_pipe):
    ...
    with patch.object(mock_pipe, "stop", new_callable=AsyncMock) as mock_stop:
        await mock_pipe._handle_commands([{"type": "stop_pipe", "datacast_pipe_id": "test-pipe"}])
        await asyncio.sleep(0.1)
