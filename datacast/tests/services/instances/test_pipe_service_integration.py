...
from datacast_core.models.config import PipeConfig, SourceConfig, SenderConfig, PasswdCredential, FieldMapping, AppConfig
...
async def test_pipe_instance_service_integration(integration_configs, tmp_path: Path, caplog):
    """Integration test for PipeManager using DatacastPipe."""
    ...
    with patch("datacast.stability.pipe_manager.DatacastConfig") as mock_DatacastConfig:
        datacast_pipe_config = DatacastPipeConfig(
            source="test_source",
            sender="test_sender",
            disabled=False,
            fields_mapping=[
        ...
        mock_DatacastConfig.get_all_pipes.return_value = {"test_pipe": datacast_pipe_config}
        mock_DatacastConfig.get_pipe.return_value = datacast_pipe_config
        mock_DatacastConfig.get_source.return_value = source_config
        mock_DatacastConfig.get_sender.return_value = sender_config
        ...
        with caplog.at_level(logging.INFO):
            try:
                ...
                # Give some time for DatacastPipe to run its sequence
                await asyncio.sleep(1)
                
                # Verify instance in pool
                instance = service.get_instance("test_pipe")
                assert instance is not None
                assert isinstance(instance, DatacastPipe)
                
                # Check logs for pipe activity
                # We assert "start initiated successfully" instead of "Using DatacastPipe"
