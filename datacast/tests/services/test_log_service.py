...
from datacast.domain.log import LogService, LOG_FILE_PATH, LOG_PATTERN
from datacast_core.models.log import LogEntry

# Mock the LOG_FILE_PATH for testing purposes
@pytest.fixture(autouse=True)
def mock_log_file_path():
    with patch('datacast.domain.log.LOG_FILE_PATH', '/tmp/test_datacast.log'):
        yield
...
def sample_log_content():
    return [
        "2025-07-08 10:00:00,123 - datacast.app - INFO - Application started.",
        "2025-07-08 10:00:01,456 - datacast.api - DEBUG - API call received.",
        "2025-07-08 10:00:02,789 - datacast.service - WARNING - Something unusual happened.",
        "2025-07-08 10:00:03,000 - datacast.driver - ERROR - Driver failed to connect.",
        "INVALID LOG LINE",
        "2025-07-08 10:00:04,111 - datacast.app - INFO - Application shutting down."
    ]

class TestLogService:
    def test_parse_valid_line(self, log_service):
        line = "2025-07-08 10:00:00,123 - datacast.app - INFO - Application started."
        entry = log_service._parse_line(line, 1)
        assert entry is not None
        assert entry.timestamp == datetime(2025, 7, 8, 10, 0, 0, 123000)
        assert entry.level == "INFO"
        assert entry.component == "datacast.app"
    ...
    def test_get_logs_with_component_filter(self, mock_file_open, mock_exists, log_service, sample_log_content):
        ...
        logs = log_service.get_logs(component="datacast.app")
        assert len(logs) == 2
        assert logs[0].component == "datacast.app"
        assert logs[1].component == "datacast.app"
        assert logs[0].message == "Application shutting down."
    ...
    def test_get_logs_with_all_filters(self, mock_file_open, mock_exists, log_service, sample_log_content):
        ...
        logs = log_service.get_logs(limit=1, level="INFO", component="datacast.app", before_line=6)
        assert len(logs) == 1
    ...
    def test_get_logs_io_error(self, mock_file_open, mock_exists, log_service):
        ...
        # with self.assertLogs('datacast', level='ERROR') as cm:
