...
from datacast.config.validator import ConfigValidator
from datacast.config.unified import DatacastConfigLoader, SourceConfig, SenderConfig, DatacastPipeConfig # Assuming these models are used internally

@pytest.fixture
def mock_loader():
    """Fixture for a mocked DatacastConfigLoader."""
    loader = MagicMock(spec=DatacastConfigLoader)
    ...
    loader.get_all_pipes.return_value = {
        "p1": DatacastPipeConfig(source="s1", sender="se1", id="p1")
    }
    return loader
...
def test_validate_pipe_redundant_pair(mock_loader):
    ...
    mock_loader.get_all_pipes.return_value["p2"] = DatacastPipeConfig(source="s1", sender="se1", id="p2")
    validator = ConfigValidator(loader=mock_loader)
    is_valid, errors = validator.validate()
    assert is_valid is False
...
def test_validate_config_dict_datacast_id_removed_from_config():
    """Test that datacast_id is no longer expected in the config dictionary."""
    config_dict = {
        "sources": {"s1": {"driver": "fs", "uri": "/tmp"}},
    ...
    from datacast.config.unified import UnifiedDatacastConfig
    cfg = UnifiedDatacastConfig.model_validate(config_dict)
    assert not hasattr(cfg, "datacast_id")
