...
class ConfigError(SensorException):
...
class NotFoundError(SensorException):
...
class ViewNotReadyError(SensorException):
    """Raised when a view is not ready (no leader or initial snapshot in progress)."""
    pass


class ConflictError(SensorException):
...
class DriverError(SensorException):
...
class StateConflictError(SensorException):
...
class ValidationError(SensorException):
...
class SessionObsoletedError(SensorException):
...
class fustordConnectionError(SensorException):
