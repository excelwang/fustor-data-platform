...
### Hot Upgrade
- **HOT_UPGRADE_ATOMICITY**: System MUST guarantee atomic, in-place replacement of the datacast process logic.
...
- **TARGETED_DELIVERY**: fustord MUST be able to target a specific datacast instance for upgrade, ignoring others sharing the same Session.
  > Responsibility: Operations — Canary deployments.
  > Verification: Only the targeted datacastID receives the upgrade command.
...
- **ZOMBIE_DETECTION**: fustord MUST detect datacasts that maintain Heartbeat but fail to send Data events for > `threshold` period.
  > Responsibility: Health — Detect "brain-dead" datacasts.
  > Verification: Alert triggered when heartbeat ok but data flow zero for X minutes.

- **REMOTE_REMEDIATION**: fustord MUST be able to push a "Kill & Restart" or "Clean Slate Config" command to a Zombie datacast.
  > Responsibility: Survival — Remote fix for non-responsive data planes.
  > Verification: Zombie datacast receives command, terminates, restarts, and resumes normal operation.
