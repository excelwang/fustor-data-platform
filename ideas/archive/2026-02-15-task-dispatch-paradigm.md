...
| **逻辑目标** | **View** (数据视图) | **datacast** (物理进程) |
| **寻址映射** | `ViewID -> List[SessionID]` | `datacastID -> List[SessionID]` |
...
*   **逻辑流程**:
    1.  解析 datacast 的 `task_id` 获取 `datacastID`。
    2.  在全局 Session 表中过滤出属于该 `datacastID` 的所有 `Session`。
    3.  **选路逻辑**:
        *   优先选择 `Leader` Session。
        *   若无 Leader，选择第一个探测到的活跃 Session（Heartbeat 最新的）。
    4.  **单点分发**: 只向选中的**一个** Session 发送指令。
    5.  **原子执行**: 利用 datacast 进程的继承关系，实现全局重启或加载。
...
class TaskOrchestrator:
    ...
    async def datacast_targeted_dispatch(self, datacast_id: str, cmd: Dict) -> Dict:
        """
        精准选路逻辑：用于升级/停止。
        保证进程级操作的原子性，避免冲突。
        """
