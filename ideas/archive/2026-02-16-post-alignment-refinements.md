...
> **Context**: Following the successful Phase 7 alignment of `datacast` with `fustord`, several opportunities for further consolidation and refinement have emerged. These aim to reduce code duplication, standardize testing patterns, and clarify runtime states.

## Proposed Changes

### 1. Consolidate Pipe Lifecycle Logic
With `datacast/stability/pipe_manager.py` now mirroring `fustord`'s structure, the shared lifecycle logic (start/stop/restart/error handling) should be extracted into `datacast-core` (formerly `fustor-core`).

- **Action**: Create `datacast_core.stability.base_manager.BasePipeManager`.
- **Action**: Extract common mixins from `datacast/stability/mixins` and `fustord/stability/mixins` to `datacast_core`.

### 2. Formalize Global Configuration Pattern
During Phase 7, we shifted `datacast` to use the global `DatacastConfig` loader for mutations. This pattern simplifies service signatures but requires rigorous testing support.
...
Tests for `PipeManager` required complex mocking of `EventBusManager` and `DatacastConfig`.

- **Action**: Create a unified `datacast_test_fixture` in `tests/conftest.py` that pre-mocks the global config and event bus with sensible defaults.
