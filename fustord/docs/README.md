...
fustord 是 Fustor 平台的核心存储与查询引擎。它负责接收来自 datacast 的异步事件流，并在内存中构建高效的元数据哈希索引，为上层应用提供极速的结构化查询能力。
...
用于维护 datacast 与 fustord 之间的同步契约。

#### **POST `/ingestor-api/v1/sessions/`**
创建新的同步会话。

*   **参数**: `task_id` (唯一任务标识)。
*   **特性**: 采用 **先到先得 (First-Come-First-Serve)** 模式，第一个建立 Session 的 datacast 成为 Leader。
...
*   第一个建立 Session 的 datacast 成为 Leader，拥有 Snapshot/Audit/Sentinel 权限。
*   后续连接的 datacast 自动成为 Follower，仅执行 Realtime Sync。
...
*   **Blind-spot List**：标记在无 datacast 客户端发生变更的文件 (`Datacast_missing: true`)

仲裁原则：**Realtime 优先，Mtime 仲裁**。详见 `docs/CONSISTENCY_DESIGN.md`。

### 3. 心跳存续依赖 (Heartbeat Availability Dependency)
为了防止权威 datacast 崩溃导致视图陈旧（Stale Data Risk）：
*   **硬链接可用性**：一旦权威 datacast 的 **心跳丢失 (Heartbeat Timeout)**，fustord 必须立即将对应的 View 状态切换为 **503 Service Unavailable**。
*   **逻辑理由**：心跳丢失意味着实时事件流（inotify）可能已中断，此时 fustord 看到的树结构不再是对物理事实的准确感知。

## 性能优化建议

*   **批量推送**: 建议 datacast 每 1000 行聚合为一个 Event 发送，以最大化摄取效率。
