...
from datacastst_core.transport.receiver import Receiver
from datacastst_core.stability import BasePipeManager, StartResult
...
class FustordPipeManager(BasePipeManager[FustordPipe], ManagerLifecycleMixin, ManagerCallbacksMixin):
    ...
    async def start_one(self, id: str, **kwargs) -> StartResult:
        try:
            ...
            if pipe:
                await pipe.start()
                return StartResult(datacastst_pipe_id=id, success=True)
            else:
                return StartResult(datacastst_pipe_id=id, success=False, error="Initialization failed silently")
        except Exception as e:
            logger.error(f"Failed to start pipe {id}: {e}")
            return StartResult(datacastst_pipe_id=id, success=False, error=str(e))
