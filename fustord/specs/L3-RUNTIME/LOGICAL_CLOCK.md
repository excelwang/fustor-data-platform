...
**fustord** 实现了基于 **影子水位线 (Logical Watermark)** 的仲裁逻辑：
...
1.  **疑似排除**: 水位线附近的事件被标记为 `Suspect`（参见 CONSISTENCY.md）。
2.  **强制校准**: 在接收到 `audit_end` 信号后, View 会强制同步一次该 Session 的影子时钟到汇报者的物理物理时间点。
"撑大"，导致所有正常时间的新文件被误判为"旧数据" |
