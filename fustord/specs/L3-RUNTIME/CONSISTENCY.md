...
Fustor 采用 **Leader/Follower** 会话对齐模型。对于特定的 View，同一时间仅有一个 **Sensord** 示例被指派为 Leader 以执行 Snapshot 和 Audit 对账任务，其余实例作为 Follower 仅同步 realtime 实时增量。
...
## 3. 核心一致性机制 (Consistency Mechanisms)

### 3.1 墓碑机制 (Tombstones)
- **场景**: 处理文件删除。
- **逻辑**: 收到删除事件时并不物理删除记录，而是标记为 Tombstone。
...
## 4. 交互原语 (SDP Primitives)

| 原语 | 说明 |
|------|------|
| **`on_modified`** | 文件内容发生变更，`is_atomic_write=False`。 |
| **`on_closed`** | 文件写关闭，`is_atomic_write=True`（视为最终状态）。 |
| **`audit_start / end`** | 标记一个物理对账周期的开始与结束。 |
sensord 的节点产生的文件变更无法实时感知 |
