...
在 **fustord** 内部，对于每一个逻辑视图（由 `ViewID` 标识），承载着来自多个 **Datacast** 节点的数据流。
...
2.  **优雅降级**: 若底层存储驱动失效，View 应标记自己为 `Stale` 并告知 Datacast 降低同步频率。
...
View 通过 **SCP** 控制面响应向 **Datacast** 下发 `scan` 或 `audit` 指令。
