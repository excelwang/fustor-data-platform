...
- **IDEMPOTENT_INGESTION**: fustord MUST ensure SDP event ingestion is idempotent. Duplicate batches from sensors MUST NOT create duplicate tree entries.
...
- **INV_API_NEVER_503**: The `/fs/tree` API endpoint MUST always return a valid response, never 503.
  ...
  > Verification: API returns data (even if marked as blind-spot) during network partitions or sensor downtime.
...
- **TARGETED_UPGRADE**: fustord MUST be able to target a specific `sensord_id` for upgrade commands via SCP unicast.
  > Responsibility: Operations.

- **ZOMBIE_DETECTION**: fustord MUST detect Sensord nodes that maintain SCP heartbeats but fail to send SDP data events for a configured threshold period.
